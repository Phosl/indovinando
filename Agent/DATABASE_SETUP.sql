...
CREATE TABLE game_bottle_answers (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_game_question_options_question_id ON game_question_options(question_id);
CREATE INDEX idx_game_bottles_game_id ON game_bottles(game_id);
CREATE INDEX idx_game_bottle_answers_bottle_id ON game_bottle_answers(bottle_id);
CREATE INDEX idx_game_bottle_answers_question_id ON game_bottle_answers(question_id);
...
-- Policy: Chiunque può leggere i giochi pubblicati
CREATE POLICY "Games are readable by everyone"
  ON games FOR SELECT
  USING (status = 'published' OR created_by = auth.uid());

-- Policy: Solo il creatore può modificare/eliminare i propri giochi
...
-- Policy: Solo il creatore può inserire giochi
CREATE POLICY "Games are creatable by auth users"
  ON games FOR INSERT
  WITH CHECK (auth.uid() = created_by);

-- Le altre tabelle seguono i giochi (se puoi leggere il gioco, puoi leggere i relativi dati)
...
-- Policies di creazione/modifica solo per il creatore del gioco
...
-- DELETE Policies for game creator to modify game content
